...
if __name__ == "__main__":
    ...
    tests = {
            ...
            'all': {
                ...
                'label_key': [f'corruption{i}' for i in range(len(corruptions))],
    ...
    # path_driller = Path('/srv/newpenny/XAI/generated_data/AE_sentinel/drillers_medium_RW/classifier.test.encoder.linear.RW_RW.medium.high.GMM.nl_model=2.n_features=50.nl_class=50')

    # pred = torch.load('pred.pt').to(device)

    # hist_counts = torch.bincount(pred, minlength=n_cluster)

    # # If you want a probability histogram instead of counts:
    # hist_probs = hist_counts.float() / pred.numel()

    # empps = []
    # for i in range(4):
    #     empp = torch.load(path_driller/f'empp_RW{i}.pt').to(device)
    #     print(empp[:,1].shape)
    #     empps.append(empp[:,1])
    # fig, axs = plt.subplots(1,2, figsize=(5,5))
    # e = torch.stack(empps, dim=1)
    # axs[0].imshow(e.detach().cpu().numpy())
    # axs[1].imshow(hist_probs.unsqueeze(dim=1).detach().cpu().numpy())
    # axs[0].set_xticks([])
    # axs[1].set_xticks([])
    # axs[1].set_yticks([])
    # plt.subplots_adjust(wspace=0, hspace=0)  # remove horizontal & vertical gaps
    # plt.tight_layout(pad=0)
    
    # fig.savefig('prova_empp.png')
    ...
    for idx in tqdm(peaks):
        ...
        for test_name in tqdm(tests):
            ...
            with ExitStack() as stack:
                ...
                for label_key in tests[test_name]['label_key']:
                    stack.enter_context(peepholes[label_key])
                    peepholes[label_key].load_only(
                                            loaders = tests[test_name]['loaders'],
                                            verbose = verbose
                                        )
                ...
                scores = {key: loss(dss['data'], dss['output']).mean(dim=(2,3)) for key, dss in sentinel._dss.items()}

                sample_mask = (sentinel._dss['test_ori']['label'] == 0).all(dim=(1,2))

                data = sentinel._dss['test_ori']['data'][sample_mask][window].squeeze(dim=1).permute(0,2,1)
                ...
                pm = torch.stack([peepholes[label_key]._phs['test_ori']['encoder.linear']['peepholes'][:,1] for label_key in tests[test_name]['label_key']], dim=1)
                pm = pm[window]
                ...
                plt.savefig(f'anomaly_{idx}_{test_name}_all.png', bbox_inches='tight')
                plt.close(fig)

                cns = tests[test_name]['class_names']

                result = torch.stack([peepholes[label_key]._phs[f'{fit}-test-c-all-{ci}']['encoder.linear']['peepholes'][:,1] for label_key in tests[test_name]['label_key']], dim=1)

                
                label = sentinel._dss[f'{fit}-test-c-all-{ci}']['corruption']
                

                idx = sentinel._dss[f'{fit}-test-c-all-{ci}']['detection'] == 1  
                ...
                # save and close
                plt.savefig(Path(plots_path)/f"CM.{fit}.{test_name}.{n_cluster}.{cv_dim}.{emb_size}.{ci}_all_only_test.png", bbox_inches='tight', dpi=300)
                plt.close()
                quit()
