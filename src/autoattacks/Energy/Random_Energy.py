...
plot_base = "/srv/newpenny/XAI/generated_data/Kami_attacks/CIFAR100_WRN28-robust/random_directions"
...
def energy(mat, indices):
    """Sum of squared projections per sample. Shape: (n_samples,)"""
    return (mat[:, indices] ** 2).sum(axis=1)

def mean_clean_energy_per_dir(mat_clean, indices):
    """Mean over samples AND directions: scalar summarising how much
    energy clean inputs carry per direction in this subspace."""
    return float((mat_clean[:, indices] ** 2).mean())
...
for mode in ('tail', 'head'):
    ...
    plt.suptitle(f'Energy KDE — {mode} {n_dirs} directions (WRN28-robust)', fontsize=13)
    plt.tight_layout()
...
rand_index_sets = [
    np.sort(rng.choice(500, size=n_dirs, replace=False))
...
for run_i, rand_idx_full in enumerate(rand_index_sets):
    ...
    for idx, layer in enumerate(layers):
        ...
        mce = mean_clean_energy_per_dir(c, rand_idx)
        plot_kde(axes[idx], c_e, ce_e, t_e, auc_ce, auc_t,
                 f"{layer}\n(mean_clean_energy_per_dir={mce:.4f})")

    for idx in range(n_layers, len(axes)):
        axes[idx].set_visible(False)

    plt.suptitle(f'Energy KDE — random run {run_i+1:02d}/{n_runs} (WRN28-robust)', fontsize=13)
    plt.tight_layout()
    plt.subplots_adjust(top=0.96)
...
# ══════════════════════════════════════════════════════════════════════
# PASS 3 — Scatter: AUC vs mean clean energy per direction
# ══════════════════════════════════════════════════════════════════════
print("\n── Scatter plot ──")
...
for idx, layer in enumerate(layers):
    ...
    all_results[layer].append(dict(
        x      = mean_clean_energy_per_dir(c, sel),
        auc_ce = auc_score(c_e, ce_e),
        auc_t  = auc_score(c_e, t_e),
        kind   = 'tail',
    ))
    ...
    for _ in range(n_runs):
        ...
        all_results[layer].append(dict(
            x      = mean_clean_energy_per_dir(c, rand_idx),
            auc_ce = auc_score(c_e, ce_e),
            auc_t  = auc_score(c_e, t_e),
            kind   = 'random',
        ))
...
for idx, layer in enumerate(layers):
    ...
    ax.scatter([p['x']      for p in rand_pts],
    ...
    xs = np.array([p['x'] for p in rand_pts])
    ...
    for kind, color in [('tail', 'tomato'), ('head', 'steelblue')]:
        ...
        ax.scatter(pt['x'], pt['auc_ce'],
                   color=color, marker='*', s=220, zorder=6,
                   label=f"{kind} ce={pt['auc_ce']:.2f}")
        ax.scatter(pt['x'], pt['auc_t'],
                   color=color, marker='^', s=130, zorder=6,
                   label=f"{kind} t={pt['auc_t']:.2f}")

    # dynamic x-axis range with a little padding
    all_x = [r['x'] for r in recs]
    x_lo, x_hi = min(all_x), max(all_x)
    pad = (x_hi - x_lo) * 0.1 if x_hi > x_lo else 0.01
    ax.set_xlim(x_lo - pad, x_hi + pad)

    ax.set_title(layer, fontsize=7)
    ax.set_xlabel('mean clean energy per direction', fontsize=6)
...
plt.suptitle('AUC vs mean clean energy per direction — random 100-direction probes (WRN28-robust)', fontsize=12)
plt.tight_layout()
plt.subplots_adjust(top=0.96)
out = f"{plot_base}/scatter_auc_vs_mean_clean_energy_per_dir.png"
plt.savefig(out, dpi=150, bbox_inches='tight')
plt.close()
print(f"Saved → {out}")
print("\nDone. Outputs:")
...
print(f"  scatter_auc_vs_mean_clean_energy_per_dir.png")