...
for idx, layer in enumerate(layers):
    ...
    # read and filter all three with same mask
    c  = clean_cv[layer][:].float()[good_mask]
    ce = apgd_ce_cv[layer][:].float()[good_mask]
    t  = apgd_t_cv[layer][:].float()[good_mask]

    # tail energy
    c_energy  = (c[:,  -tail:] ** 2).sum(dim=1).numpy()
    ce_energy = (ce[:, -tail:] ** 2).sum(dim=1).numpy()
    t_energy  = (t[:,  -tail:] ** 2).sum(dim=1).numpy()

    # compute AUC — clean=0, adversarial=1
    ...
    ax.set_xlabel('tail energy', fontsize=6)
...
plt.suptitle(f'Tail Energy - WRN28-robust', fontsize=13)
plt.tight_layout()
plt.subplots_adjust(top=0.96)
save_path = f"{plot_base}/tail_energy_kde_auc_robust_filtered.png"
