...
LAYER = "block1.layer.2.conv2" # target layer block1.layer.2.conv2
TAIL = 100

TARGET_CLEAN_ACC = 0.7257
...
def plot_threshold(clean_energy, attack_energy, threshold, target_fpr):
    ...
    plt.title(
        ...
        f"{MODEL}, target clean FPR = {target_fpr:.2%}"
    )
...
implied_fpr = 1.0 - (TARGET_CLEAN_ACC / base_clean_accuracy)

threshold = np.quantile(clean_energy, 1.0 - implied_fpr)
...
print(f"Target clean accuracy:         {TARGET_CLEAN_ACC:.4f}")
print(f"Implied target FPR:            {implied_fpr:.4f}")
...
print(f"Target clean accuracy:  {TARGET_CLEAN_ACC:.2%}")
print(f"Implied target FPR:     {implied_fpr:.2%}")
