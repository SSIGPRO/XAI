...
if __name__ == "__main__":
    ...
    class_names = ['mirror' ,'basket', 'ladybug', 'ladybird', 'dragonfly', 'reptile', 'mountain', 'human', 'amphibian', 'mammal', 'fish', 'brown object', 'wood', 'seaside', 'bed', 'countryside', 'bird', 'horse', 'skyscraper', 'white background', 'line', 'geometric form', 'dog', 'white and brown dog' ]
    ...
    with corevecs as cv:
        ...
        cluster = 9
        idx = torch.argwhere((clusters==cluster) & (conf>0.9)).squeeze()
        ...
        for score, idx in zip(values, indices):
            print(f"{class_names[idx]}: {score.item():.3f}")
        ...
        fig.suptitle(f"{class_names[similarity[0].topk(1)[1]]}: cluster population {len(images)}", fontsize=20, y=1.02)
