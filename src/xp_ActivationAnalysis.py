...
#     print(f"Using {device} device")
    ...
    with ExitStack() as stack:
        ...
        print('CoreVectors activations shape: ', cv._dss['val']['in_activations']['classifier.6'].shape)
        act_data = cv._dss['val']['in_activations']['classifier.6']
        act_ori = act_data.clone()
        label = torch.zeros(act_data.shape[0], dtype=torch.long)
        tsne = TSNE(
                n_components=2,
                perplexity=30,      # you can try values like 5, 30, 50
                learning_rate='auto',
                init='random',      # or 'pca'
                random_state=42
                )

        for i, (atk_name, cv_atk) in enumerate(cv_atk.items()):
                if verbose: print(f'\n---------\nLoading dataset for attack: {atk_name}')
                stack.enter_context(cv_atk) # enter context manager
                cv_atk.parse_ds(
                        batch_size = bs,
                        datasets = atk_dss,
                        n_threads = n_threads,
                ...
                act_attack = cv_atk._dss['val']['in_activations']['classifier.6']
                
                act_1vs1 = torch.vstack((act_ori, act_attack))
                label_1vs1 = torch.hstack((torch.zeros(act_ori.shape[0], dtype=torch.long),
                                          torch.ones(act_attack.shape[0], dtype=torch.long) * (i+1)))
                act_tsne = tsne.fit_transform(act_1vs1)

                plt.figure(figsize=(12, 10))
                scatter = plt.scatter(act_tsne[:, 0], act_tsne[:, 1], c=label_1vs1.detach().cpu().numpy(), cmap='viridis', s=1, alpha=0.4)
                plt.colorbar(scatter, ticks=np.unique(label.detach().cpu().numpy()))
                plt.title("t-SNE Visualization of Features")
                plt.xlabel("t-SNE Dimension 1")
                plt.ylabel("t-SNE Dimension 2")
                plt.grid(True)
                plt.show()
                plt.savefig(f'Viz_activations_oriVS{atk_name}.png')

                act_data = torch.vstack((act_data, cv_atk._dss['val']['in_activations']['classifier.6']))
                label = torch.hstack((label, torch.ones(cv_atk._dss['val']['in_activations']['classifier.6'].shape[0], dtype=torch.long) * (i+1)))
                
        
        act_tsne = tsne.fit_transform(act_data)

        plt.figure(figsize=(12, 10))
        scatter = plt.scatter(act_tsne[:, 0], act_tsne[:, 1], c=label.detach().cpu().numpy(), cmap='viridis', s=1, alpha=0.4)
        plt.colorbar(scatter, ticks=np.unique(label.detach().cpu().numpy()))
        plt.title("t-SNE Visualization of Features")
        plt.xlabel("t-SNE Dimension 1")
        plt.ylabel("t-SNE Dimension 2")
        plt.grid(True)
        plt.show()
        plt.savefig(f'Viz_activations_oriVSall.png')
    '''
    with ExitStack() as stack:
        stack.enter_context(cv)
        ...
        for l in range(n_classes):
                ...
                act_data = cv._dss['val']['in_activations']['classifier.6'][l == cv._dss['val']['label']]
                fig, axs = plt.subplots(1, 4, figsize=(12, 10))

                for i, (atk_name, cva) in enumerate(cv_atk.items()):
                        if verbose: print(f'\n---------\nLoading dataset for attack: {atk_name}')
                        stack.enter_context(cva) # enter context manager
                        cva.parse_ds(
                                batch_size = bs,
                                datasets = atk_dss,
                                n_threads = n_threads,
                        ...
                        act_attack = cva._dss['val']['in_activations']['classifier.6'][l == cv._dss['val']['pred']]

                        
                        act_1vs1 = torch.vstack((act_data, act_attack))
                        label_1vs1 = torch.hstack((torch.zeros(act_data.shape[0], dtype=torch.long),
                                                torch.ones(act_attack.shape[0], dtype=torch.long) * (i+1)))
                        act_tsne = tsne.fit_transform(act_1vs1)

                        
                        scatter = axs[i].scatter(act_tsne[:, 0], act_tsne[:, 1], c=label_1vs1.detach().cpu().numpy(), cmap='viridis', s=1, alpha=1)
                        fig.colorbar(scatter, ax=axs[i], ticks=np.unique(label_1vs1.detach().cpu().numpy()))
                        axs[i].set_title(f"{atk_name} ")
                        axs[i].set_xlabel("t-SNE Dimension 1")
                        axs[i].set_ylabel("t-SNE Dimension 2")
                        axs[i].grid(True)
                        
                fig.savefig(f'Viz_activations_label={l}.png')
                if l == 5:
                     break